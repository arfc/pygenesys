...
def test_choose_growth_method_no_imput():
    ...
    if str(linear).find("linear_growth") != -1:
        ...
        raise ValueError('The wrong function was used by choose_growth_method.')
...
def test_linear_growth_unit():
    ...
    N_years = 2
    growth_rate = 1
    growth = pyutgm.linear_growth(init_value, start_year, end_year, N_years, growth_rate)
...
def test_exponential_growth_unit():
    ...
    N_years = 2
    growth_rate = 1
    growth = pyutgm.exponential_growth(init_value, start_year, end_year, N_years, growth_rate)
...
def test_logistic_growth_unit():
    ...
    growth_rate = 1
    cap = 2
    growth = pyutgm.logistic_growth(init_value, start_year, end_year, N_years, growth_rate, cap)

    assert(growth == approx(np.array([1, 1.4621171572600098, 1.7615941559557646])))
